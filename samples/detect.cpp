...
int main(int argc, char *argv[]) {
    ...
    string netConfiguration = "fastcnn_deploy.prototxt";
    string pretrainedNet = "fastcnn.caffemodel";
 ...
 //   NeuralNetwork net(netConfiguration, pretrainedNet);
   // Heatmap map;
    //net.processImage(image, map);
    FastCNNDetector detector(netConfiguration, pretrainedNet);

    vector<Rect> detectedObjects;
    detector.detect(image, detectedObjects);
    cout << detectedObjects.size() << endl;
